...
S_UNDERLINE_THICKNESS = 2  # 下划线粗细。像素
...
M_VK2LOGICNAME: dict[int, str] = {
    ...
    **M_VK2MODIFIER,
    ...
    # 我键盘上没有的键我懒得写了，摆大烂，这个地方看你键盘上有啥就写啥吧
    # 虽然我键盘上有的AC(Fn+F1)和B4(Fn+F2)我也没写
    # 摆烂！！！！！
}
...
C_STROKE_OFFSET = [(dx, dy) for dx in [-S_STROKE_SIZE, 0, S_STROKE_SIZE] for dy in [-S_STROKE_SIZE, 0, S_STROKE_SIZE] if dx and dy]
...
class KeySprites:
    ...
    def set_attachments(self, modifiers: tuple[int, ...], is_modifier: bool = False, mod_name: str | None = None):
        ...
        for i, level in enumerate(modifiers):
            if level > 0:
                ...
                stroke = Line(0, 0, 0, 0, thickness=S_UNDERLINE_THICKNESS + 2 * S_STROKE_SIZE, color=S_COLOR_STROKE, batch=self.base.batch)
                self.attachment_strokes.append(stroke)
                line = Line(0, 0, 0, 0, thickness=S_UNDERLINE_THICKNESS, color=self.base.color, batch=self.base.batch)
    ...
    def move_to(self, x: int, y: int):
        ...
        for i, (dx, dy) in enumerate(C_STROKE_OFFSET):
